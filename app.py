...
def load_historical_data(file_path):
    ...
    # Convert 'Draw Date' to datetime objects first for internal calculations,
    # then store as YYYY-MM-DD string for consistency when returning to template
...
def hot_cold_numbers(df, last_draw_date_str):
    ...
    recent_data = df[df['Draw Date_dt'] >= one_year_ago] # Use the datetime column for comparison
...
def sum_of_main_balls(df):
    ...
    # Calculate frequency of sums for the histogram/chart
    sum_freq = temp_df['Sum'].value_counts().sort_index()
    sum_freq_list = [{'sum': int(s), 'count': int(c)} for s, c in sum_freq.items()]

    # Calculate summary statistics for sums
    min_sum = int(temp_df['Sum'].min())
    max_sum = int(temp_df['Sum'].max())
    avg_sum = round(temp_df['Sum'].mean(), 2)
...
def partial_match_probabilities(white_ball_range, powerball_range):
    ...
    for scenario, data in prizes.items():
        if total_powerball_comb > 0:
            ...
            if data["matched_p"] == 1:
                comb_p = calculate_combinations(total_powerballs_in_range, 1) # Must match the Powerball
            else:
                comb_p = calculate_combinations(total_powerballs_in_range - 1, 1) # Must NOT match the Powerball (pick from others)
                if total_powerballs_in_range == 1: # Edge case if only one powerball is possible
                     comb_p = 0
...
# Find last draw dates for individual numbers (used in generate/modified display)
def find_last_draw_dates_for_numbers(df, white_balls, powerball):
    last_draw_dates = {}
    
    # Sort by date descending using the datetime column for correct order
...
# NEW FEATURE: Calculate Hit and Miss for all numbers (miss streaks)
def calculate_hit_and_miss(df):
    all_draw_dates = sorted(df['Draw Date_dt'].drop_duplicates().tolist())
    
    white_ball_hit_miss = []
    for i in range(1, 70): # Numbers 1-69 for white balls
        last_appearance_date = None
        for index, row in df.iterrows():
            if i in row[['Number 1', 'Number 2', 'Number 3', 'Number 4', 'Number 5']].values:
                last_appearance_date = row['Draw Date_dt']
        
        if last_appearance_date is not None:
            # Calculate miss streak based on number of draws AFTER last appearance
            miss_streak_count = 0
            for d_date in reversed(all_draw_dates): # Iterate from most recent to find where last_appearance_date is
                if d_date == last_appearance_date:
                    break
                miss_streak_count += 1
            white_ball_hit_miss.append({'number': i, 'miss_streak': miss_streak_count})
        else:
            white_ball_hit_miss.append({'number': i, 'miss_streak': len(all_draw_dates)}) # Never drawn
    
    powerball_hit_miss = []
    for i in range(1, 27): # Numbers 1-26 for Powerball
        last_appearance_date = None
        for index, row in df.iterrows():
            if i == row['Powerball']:
                last_appearance_date = row['Draw Date_dt']
        ...
        if last_appearance_date is not None:
            ...
            for d_date in reversed(all_draw_dates):
                if d_date == last_appearance_date:
                    break
                miss_streak_count += 1
            powerball_hit_miss.append({'number': i, 'miss_streak': miss_streak_count})
        else:
            powerball_hit_miss.append({'number': i, 'miss_streak': len(all_draw_dates)}) # Never drawn

    # Sort by miss streak descending for "most overdue" at top
    white_ball_hit_miss.sort(key=lambda x: x['miss_streak'], reverse=True)
    powerball_hit_miss.sort(key=lambda x: x['miss_streak'], reverse=True)

    return white_ball_hit_miss, powerball_hit_miss

# NEW FEATURE: Get Number Age Distribution for Chart
def get_number_age_distribution(df):
    white_ball_hit_miss, powerball_hit_miss = calculate_hit_and_miss(df) # Reuse this for individual ages
    
    all_miss_streaks = [item['miss_streak'] for item in white_ball_hit_miss] + \
                       [item['miss_streak'] for item in powerball_hit_miss]

    # Count frequency of each miss streak
    age_counts = pd.Series(all_miss_streaks).value_counts().sort_index()
    
    # Format for D3.js: [{'age': 0, 'count': 5}, ...]
    number_age_data = [{'age': int(age), 'count': int(count)} for age, count in age_counts.items()]
    
    return number_age_data

# NEW FEATURE: Get Number Co-occurrence Matrix Data
def get_co_occurrence_matrix(df):
    co_occurrence = defaultdict(int)
    
    # Iterate through each row (draw)
    for index, row in df.iterrows():
        white_balls = sorted([row['Number 1'], row['Number 2'], row['Number 3'], row['Number 4'], row['Number 5']])
        # Generate all unique pairs from the 5 white balls in the draw
        for i in range(len(white_balls)):
            for j in range(i + 1, len(white_balls)):
                pair = tuple(sorted((white_balls[i], white_balls[j]))) # Ensure consistent order for pairs
                co_occurrence[pair] += 1
    
    # Convert to list of dictionaries for D3.js: [{'x': num1, 'y': num2, 'count': frequency}]
    co_occurrence_data = []
    for pair, count in co_occurrence.items():
        co_occurrence_data.append({'x': pair[0], 'y': pair[1], 'count': count})
        # For symmetric display, add the reverse pair as well, with same count
        # co_occurrence_data.append({'x': pair[1], 'y': pair[0], 'count': count}) 
        # (Decision: Let D3 handle the symmetry if needed, or only render lower triangle)
        # For a true heatmap grid, we'll need to fill in zeros for non-occurring pairs later in JS,
        # but for passing just the data, this is fine.
    
    # Max value for color scale (optional, can be calculated in JS)
    max_co_occurrence = max(co_occurrence.values()) if co_occurrence else 0
    
    return co_occurrence_data, max_co_occurrence

# NEW FEATURE: Get Powerball Position Frequency Data
def get_powerball_position_frequency(df):
    ...
    for index, row in df.iterrows():
        ...
        # Create a data point for each white ball's position relative to the powerball
        for i in range(5): # Positions 0-4, map to 1-5 for chart
            position_frequency_data.append({
                'white_ball_position': i + 1, # 1-indexed position
...
try:
    ...
    # Pre-compute common analysis data when the app starts
    precomputed_white_ball_freq, precomputed_powerball_freq = frequency_analysis(df)
    precomputed_last_draw_date_str = get_last_draw(df)['Draw Date']
    ...
    # NEW Pre-computed data for advanced features
...
def sum_of_main_balls_route():
    sum_data_df, sum_freq_list, min_sum, max_sum, avg_sum = sum_of_main_balls(df) # Recalculate as it's small data
    last_draw_dict = last_draw.to_dict()
    last_draw_dict['Draw Date'] = pd.to_datetime(last_draw_dict['Draw Date']).strftime('%Y-%m-%d')
...
# Route for the NEW Hit and Miss Analysis
@app.route('/hit_and_miss_analysis')
def hit_and_miss_analysis_route():
    # Use pre-computed data
    white_ball_hit_miss, powerball_hit_miss = precomputed_white_ball_hit_miss, precomputed_powerball_hit_miss
    
    last_draw_dict = last_draw.to_dict()
    last_draw_dict['Draw Date'] = pd.to_datetime(last_draw_dict['Draw Date']).strftime('%Y-%m-%d')

    return render_template('index.html',
                           white_ball_hit_miss=white_ball_hit_miss,
                           powerball_hit_miss=powerball_hit_miss,
                           last_draw=last_draw_dict)

# Route for the NEW Number Age Distribution Chart
...
# Route for the NEW Number Co-occurrence Heatmap
...
# Route for the NEW Powerball Position Frequency Scatter Plot
