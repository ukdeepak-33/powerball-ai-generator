...
def filter_common_pairs_by_range(common_pairs, num_range):
    ...
    for pair in common_pairs:
        if num_range[0] <= pair[0] <= num_range[1] and num_range[0] <= pair[1] <= num_range[1]:
...
def generate_with_common_pairs(df, common_pairs, white_ball_range, excluded_numbers):
    ...
    # Combine the numbers
    white_balls = [num1, num2] + remaining_numbers
    return white_balls
...
def generate_modified():
    ...
    num_range = tuple(map(int, request.form.get('num_range', '').split())) if request.form.get('num_range') else None

    # Find common pairs (if enabled)
    common_pairs = []
    if use_common_pairs:
    ...
    if use_common_pairs and common_pairs:
...
def find_results_by_first_white_ball():
    ...
    # Convert the 'Draw Date' column to datetime (if not already done)
    results['Draw Date'] = pd.to_datetime(results['Draw Date'])
